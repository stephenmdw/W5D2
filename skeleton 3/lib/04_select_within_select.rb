...
def richer_than_england
  ...
  execute(<<-SQL)
    ...
    FROM
      countries
    WHERE
      gdp/population > (SELECT gdp/population as per_capita FROM countries WHERE name = 'United Kingdom') AND continent = 'Europe'
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
    ...
    WHERE
      ...
      --(SELECT count(name) FROM countries WHERE population < 25000000 GROUP BY continent )
  SQL
end
